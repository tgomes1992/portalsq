...
class InvestidoresSinc:
    ...
    def main_sincronizar_clientes(self):
        ...
        for cliente in clientes_a_sincronizar:
            consulta_cliente = ManClienteService(os.environ.get("JCOT_USER"),os.environ.get("JCOT_PASSWORD")).request_consultar_cliente(cliente.CD_CLIENTE)
            if consulta_cliente == "EJCOT-0001":
                cadastrar_clientes = ManClienteService(os.environ.get("JCOT_USER"), os.environ.get("JCOT_PASSWORD"))
    ...
    def main_sincronizar_cotistas(self):
        ...
        for cliente in clientes_a_sincronizar:
            consulta_cotista = Mancotistav2Service(os.environ.get("JCOT_USER"),os.environ.get("JCOT_PASSWORD")).request_consultar_cotista(cliente.CD_CLIENTE)
            print(consulta_cotista)
            if consulta_cotista == "EJCOT-0001":
                cadastro_cotistas = Mancotistav2Service(os.environ.get("JCOT_USER"), os.environ.get("JCOT_PASSWORD"))
